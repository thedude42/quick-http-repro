...
util.inherits(SimpleHttpParser, EventEmitter);
    ...
    httpMessageStream.on('end', function() {
        ...
        while ( i < messagePieces.length) {
            ...
            if (messageObj.type === "request" || messageObj.type === "response") { // case: headers
                ...
                if (i === 0) {
                    throw "file "+self.inFile+" begins with an incomplete http message";
                }
                contentLength =
...
SimpleHttpParser.prototype.getBodyObj = function(bodyPieces, bodyObj) {
    var piece = bodyPieces[0],
        bodyEnd;
    if (piece === undefined) {
        console.log("last message piece token");
        return bodyObj;
    }
    if (bodyObj.contentLength) {
        if (piece.length === bodyObj.contentLength)
        {
        ...
        else if (bodyObj.bodyStr.length === bodyObj.contentLength) {
            console.log("recursive return case");
            return bodyObj;
        }
            ...
            console.log("str: "+bodyObj.bodyStr);
            if (bodyObj.bodyStr === "") {
                bodyObj.bodyStr += piece;
            }
            else {
